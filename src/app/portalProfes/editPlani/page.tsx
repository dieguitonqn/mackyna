...
const EditPlani = () => {
    ...
    const handleDeleteDay = (e: React.MouseEvent<HTMLButtonElement>, dayIndex: number) => {
        e.preventDefault();
        const updatedDays = editedPlani.trainingDays.filter((_, index) => index !== dayIndex);
        setEditedPlani({
            ...editedPlani,
        ...
        <div className='flex flex-col items-center'>
            <form onSubmit={handleSubmit}>
                ...
                <div className='mb-4'>
                    ...
                    <input type="date" value={new Date(editedPlani.startDate).toISOString().split('T')[0]} onChange={(e) => handlePlaniChange('startDate', e.target.value)} className='shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline' />
                </div>
                <div className='mb-4'>
                    <label className='block text-gray-700 text-sm font-bold mb-2'>Fecha de Fin</label>
                    <input type="date" value={new Date(editedPlani.endDate).toISOString().split('T')[0]} onChange={(e) => handlePlaniChange('endDate', e.target.value)} className='shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline' />
                </div>
                {editedPlani.trainingDays.map((trainingDay, dayIndex) => (
                    <div className='flex flex-col items-center  bg-slate-200 my-5 p-2' key={dayIndex}>
                                        ...
                                        trainingDay.Bloque1.map((exercise, exerciseIndex) => (
                                            <div key={exerciseIndex}>
                                                <ExerciseForm
                                                    ...
                                                    initialNotas={exercise.notas}
                                                    initialVideoLink={exercise.videoLink}
