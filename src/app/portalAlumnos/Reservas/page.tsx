...
async function Reservas() {
  ...
  try {
    const reservas: IReserva[] = (
      await Reserva.find({ "userInfo.userId": session.user.id })
        .sort({ "turnoInfo.dia_semana": 1, "turnoInfo.hora_inicio": 1 })
        .lean()
    ).map((reserva) => ({
      userInfo: {
        ...
        <div className="h-screen flex flex-col justify-center items-center">
          ...
          <div className="flex justify-end m-5 md:m-10">
            <Link
              ...
              href="/portalAlumnos/Turnos"
            >
              <FaRegCalendarPlus className="flex" />
              Nueva Reserva
            </Link>
          </div>
        </div>
    ...
    const reservasConTurnos = await Promise.all(
      reservas.map(async (reserva) => {
        const turnoDoc = (await Turno.findById(
          new ObjectId(reserva.turnoInfo.turnoId)
        ).lean()) as unknown as Turnos;
        return {
          ...reserva,
          turnoInfo: {
            ...reserva.turnoInfo,
              ...
              <div className="flex flex-col space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-semibold">
                    {reserva.turnoInfo.dia_semana}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-500">Hora de inicio</span>
                  <span className="font-semibold">
                    {reserva.turnoInfo.hora_inicio}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-500">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-semibold">
                    {12 - reserva.turnoInfo.cupos_disponibles}/12
                  </span>
                </div>
                <div>
                  <CancelButton
