...
async function generarComprobantePDF(payment: PaymentResponse , payerUser: IUser) {
...
export async function POST(request: Request) {
  ...
  try {
    const payment:PaymentResponse = await new Payment(mpClient).get({ id: body.data.id });
