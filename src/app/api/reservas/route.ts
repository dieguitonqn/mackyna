...
export const POST = async (req: Request) => {
  ...
  try {
    ...
    try {
      const userReservas: number = await Reserva.countDocuments({
      ...
      if (reservasMismoDia) {
        logger.warn(
          `Usuario ${body.userID} intentó reservar más de un turno en el día ${turno.dia_semana}`
        );
        return new NextResponse("Ya tienes una reserva para este día", {
          status: 403,
        });
      }
      ...
      if (userPrevReservas > 0) {
        logger.warn(
          `Usuario ${body.userID} intentó reservar el mismo turno dos veces`
        );
        return new NextResponse("Ya tienes una reserva en este turno", {
          status: 403,
        });
      }
      ...
      const dias_disp = user.dias_permitidos
      ...
      logger.info(
        `Reserva creada exitosamente para usuario ${body.userID} en turno ${body.turnoID}`
      );
