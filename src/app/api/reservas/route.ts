...
export const POST = async (req: Request) => {
  ...
  try {
    ...
    const reservasPrevias = await Reserva.find({
      "userInfo.userId": body.userID,
    });
    if (reservasPrevias.length > 0) {
      logger.warn(
        `El usuario ${body.userID} ya tiene reservas previas. Se eliminarán antes de crear nuevas reservas.`
      );
      for (const reserva of reservasPrevias) {
        await Turno.findByIdAndUpdate(reserva.turnoInfo.turnoId, {
          $inc: { cupos_disponibles: 1 },
        });
      }
    }
    // Eliminar reservas previas del usuario
    if (body.individual === false || body.individual === undefined) {
      const limpiarReservasPrevias = await Reserva.deleteMany({
        "userInfo.userId": body.userID,
      });
      if (limpiarReservasPrevias.deletedCount === 0) {
      logger.warn(
        `No se encontraron reservas previas para el usuario: ${body.userID}`
      );
    }
    }
    ...
    const turnoIDs: string[] = Array.isArray(body.turnoIDs)
      ? body.turnoIDs
      : body.turnoID
      ? [body.turnoID]
      : [];
    ...
    let userReservas: number = await Reserva.countDocuments({
      "userInfo.userId": body.userID,
    });
    const resultados: any[] = [];
    let dias_disp = user.dias_permitidos
      ? user.dias_permitidos - userReservas
      : undefined;
      ...
      try {
        ...
        if (reservasMismoDia) {
          resultados.push({
            turnoID,
            status: 403,
            message: `Ya tienes una reserva para el día ${turno.dia_semana}`,
          });
          continue;
        }
        ...
        if (userPrevReservas > 0) {
          resultados.push({
            turnoID,
            status: 403,
            message: "Ya tienes una reserva en este turno",
          });
          continue;
        }
        // Verificar límite de reservas del usuario
        if (
          user.dias_permitidos !== undefined &&
          userReservas >= user.dias_permitidos
        ) {
          resultados.push({
            turnoID,
            status: 401,
            message: "Exceso de reservas",
          });
          continue;
        }
        ...
        userReservas++;
        dias_disp = user.dias_permitidos
          ? user.dias_permitidos - userReservas
          : undefined;
        resultados.push({
          turnoID,
        ...
        logger.error(
          `Error al procesar la reserva para turno ${turnoID}: ${error}`
        );
        resultados.push({
          turnoID,
          status: 500,
    ...
    logger.info(
      `Reservas procesadas para usuario ${body.userID}: ${JSON.stringify(
        resultados
      )}`
    );
    return new NextResponse(JSON.stringify({ resultados, dias_disp }), {
      status: 200,
    });
  } catch (error: unknown) {
...
export const DELETE = async (req: Request) => {
  ...
  try {
    ...
    // if (diasSemana.indexOf(turno.dia_semana.toLocaleLowerCase()) >= diasSemana.indexOf(diaActual)) {
    // Permitir cancelación
    const reservaC = await Reserva.deleteOne({
      "turnoInfo.turnoId": body.turnoID,
      "userInfo.userId": body.userID,
    });
    if (reservaC.deletedCount === 0) {
      return new NextResponse(
        JSON.stringify({ mensaje: "Reserva no encontrada" }),
        { status: 404 }
      );
    }

    // Incrementar cupos disponibles
    await Turno.findByIdAndUpdate(body.turnoID, {
      $inc: { cupos_disponibles: 1 },
    });
    return new NextResponse("Reserva cancelada correctamente", { status: 200 });
    // }
    ...
    return new NextResponse("Error al cancelar la reserva: " + error, {
      status: 500,
    });
  }
};
...
//   console.log("Obteniendo reservas en server para el usuario:", userID);
  if (userID) {
    try {
      await connect();
      const reservasData = await Reserva.find({
        "userInfo.userId": userID,
      }).lean();
      console.log("Reservas encontradas para el usuario:", reservasData.length);
      const reservas = await Promise.all(
        reservasData.map(async (doc) => {
          const userHab = await User.findOne({
            _id: new ObjectId(doc.userInfo.userId.toString() as string),
            habilitado: true,
          });
          if (userHab) {
            return {
